...
- `groupby()`로 특정 컬럼을 기준으로 데이터를 묶어서 비교할 수 있다.
- 0과 1로 된 컬럼은 `mean()`을 계산하면 1의 비율을 구할 수 있다.
- `agg()`로 여러 통계값을 한 번에 계산할 수 있다.
...
- `Embarked` 결측치는 2개뿐이다.
- 전체 생존율은 약 38.4%다.
- 여성 승객은 314명 중 233명이 생존했고, 생존율은 약 74.2%다.
- 남성 승객은 577명 중 109명이 생존했고, 생존율은 약 18.9%다.
- 성별에 따라 생존율 차이가 크게 나타나므로, `Sex`는 생존 여부와 관련이 큰 컬럼으로 보인다.
- 1등석 승객은 216명 중 136명이 생존했고, 생존율은 약 63.0%다.
- 2등석 승객은 184명 중 87명이 생존했고, 생존율은 약 47.3%다.
- 3등석 승객은 491명 중 119명이 생존했고, 생존율은 약 24.2%다.
- 객실 등급이 높을수록 생존율이 높게 나타났다.
- `Pclass`도 생존 여부와 관련이 큰 컬럼으로 보인다.

## Next

- 나이 분포 시각화
- 성별과 객실 등급을 함께 묶어서 생존율 확인
